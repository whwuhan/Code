...
void renderCube();
void renderQuad(); //渲染地面

// settings
const unsigned int SCR_WIDTH = 800;
const unsigned int SCR_HEIGHT = 600;
bool shadows = true; //是否产生阴影
...
int main()
{
    ...
    {
        // 设置depth cubemap texture的属性 最后一个参数纹理的值
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //转换矩阵 包含projection * view （注意没有包含model矩阵）
        std::vector<glm::mat4> shadowTransforms;
        shadowTransforms.push_back(shadowProj * glm::lookAt(lightPos, lightPos + glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f)));  //right
        shadowTransforms.push_back(shadowProj * glm::lookAt(lightPos, lightPos + glm::vec3(-1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f))); //left
        ...
        shadowTransforms.push_back(shadowProj * glm::lookAt(lightPos, lightPos + glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, -1.0f, 0.0f)));  //near
        shadowTransforms.push_back(shadowProj * glm::lookAt(lightPos, lightPos + glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, -1.0f, 0.0f))); //far
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
            ...
            for (unsigned int i = 0; i < 6; ++i)
            {
                //给着色器中的变换矩阵赋值（注意变换矩阵没有包括model矩阵）
...
void renderScene(const Shader &shader)
{
    ...
    glDisable(GL_CULL_FACE);             //因为是在room cube的里面渲染的画面
