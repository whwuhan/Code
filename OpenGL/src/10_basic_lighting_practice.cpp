...
int main(){
    ...
    ="/Users/wuhan/wuhan/coding_space/Code/opengl/shader/10/10.basic_lighting1.vs.glsl";
    std::string colors_fragment_path
    ="/Users/wuhan/wuhan/coding_space/Code/opengl/shader/10/10.basic_lighting1.fs.glsl";

    //光源着色器
    std::string light_cube_vertex_path
    ="/Users/wuhan/wuhan/coding_space/Code/opengl/shader/10/10.light_cube1.vs.glsl";
    std::string light_cube_fragment_path
    ="/Users/wuhan/wuhan/coding_space/Code/opengl/shader/10/10.light_cube1.fs.glsl";
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        //移动光源（注意是通过model矩阵去改变光照物体的位置，而不是直接改变光照物体的位置坐标）
        float radius = 5.0f;//环绕半径
        //更改光源的位置
        float time = glfwGetTime();
        float lightPosX = sin(time) * radius;//光源X坐标
        float lightPosZ = cos(time) * radius;//光源Y坐标
        lightPos.x = lightPosX;
        //lightPos.y = lightPosZ;
        lightPos.z = lightPosZ;
        model = glm::mat4(1.0f);//重新建立模型矩阵
        model = glm::translate(model, lightPos);//平移光源
        model=glm::scale(model,glm::vec3(0.2f));//缩放光源物体
        lightCubeShader.setMat4("model",model);
