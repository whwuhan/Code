...
int main()
{
    ...
    //OpenGL为我们定义了很多个选项，我们将在下面列出大部分最常用的选项。注意常数颜色向量C¯constant可以通过glBlendColor函数来另外设置。
    // GL_ZERO	因子等于0
    // GL_ONE	因子等于1
    ...
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);  //设置颜色混合方程的透明度
    

    //着色器
    std::string vertex_path="/Users/wuhan/wuhan/coding_space/Code/opengl/shader/22/22.blending.vs.glsl";
    std::string fragment_path="/Users/wuhan/wuhan/coding_space/Code/opengl/shader/22/22.blending.fs.glsl";
    ...
    std::string tex_window_path="/Users/wuhan/wuhan/coding_space/Code/opengl/img/blending_transparent_window.png";
    ...
    //设置纹理超出范围 重复边缘图像，否则设置为GL_REPEAT会产生一个细小的透明框
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // windows (from furthest to nearest) 用的反向迭代器，所以是由远及近
        glBindVertexArray(transparentVAO);
