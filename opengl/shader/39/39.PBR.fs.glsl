...
// material parameters
uniform vec3 albedo;        // 类似于diffuse
uniform float metallic;     // 理解为含金属的量
uniform float roughness;    // 粗糙程度
uniform float ao;           // 环境光遮蔽 ambient occlusion
...
void main()
{
    ...
    vec3 F0 = vec3(0.04); // 物体表面基础的反射能力
    F0 = mix(F0, albedo, metallic); // mix(x, y, a) return (1 - a) * x + a * y

    // reflectance equation 反射方程 也就是最外面的那个方程
    vec3 Lo = vec3(0.0);  // light out
    for(int i = 0; i < lightPositions.length(); ++i) //4个光源
    {
